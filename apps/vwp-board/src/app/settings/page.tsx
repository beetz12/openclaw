...
export default function SettingsPage() {
  ...
  const fetchTeam = useCallback(async () => {
    try {
      ...
      const msg = err && typeof err === "object" && "error" in err ? (err as { error: string }).error : "Failed to load team";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8">
      ...
      {error && (
        <div className="mb-4 rounded-[var(--radius-sm)] border border-red-300 bg-red-50 p-3 text-sm text-red-700">
          {error}
        </div>
      )}
      ...
      <section className="mb-8">
        <h2 className="text-lg font-semibold text-[var(--color-text)] mb-4">Team Management</h2>
        {team && (
            ...
            editingId === member.id ? (
              ...
              <div key={member.id} className={`${cardClass} flex items-center justify-between`}>
                <div className="min-w-0 flex-1">
                  ...
                  {member.description && (
                    <div className="mt-1 text-sm text-[var(--color-text-secondary)]">{member.description}</div>
                  )}
                  {member.skills.length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-1">
                      {member.skills.map((skill) => (
                        <span key={skill} className="rounded-full bg-[var(--color-bg-subtle)] px-2 py-0.5 text-xs text-[var(--color-text-secondary)]">
                          {skill}
                        </span>
                      ))}
                    </div>
